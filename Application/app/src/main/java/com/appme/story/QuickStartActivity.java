...
import com.appme.story.databinding.ActivityQuickStartBinding;

/**
 * A minimal example of saving and loading username/password credentials from the Credentials API.
 * @author samstern@google.com
 */
public class QuickStartActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityQuickStartBinding.inflate(getLayoutInflater());
        ...
        // Buttons
        binding.buttonSaveCredential.setOnClickListener(this);
        binding.buttonLoadCredentials.setOnClickListener(this);
        binding.buttonLoadHint.setOnClickListener(this);
        binding.buttonDeleteLoadedCredential.setOnClickListener(this);
...
;
        ...
        try {
            startIntentSenderForResult(intent.getIntentSender(), RC_HINT, null, 0, 0, PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_MUTABLE);
            mIsResolving = true;
    ...
    private void processRetrievedCredential(Credential credential, boolean isHint) {
        ...
        if (!isHint) {
            ...
            binding.buttonDeleteLoadedCredential.setEnabled(true);
        } else {
            showToast("Credential Hint Retrieved");
        }

        binding.editTextEmail.setText(credential.getId());
        binding.editTextPassword.setText(credential.getPassword());
    ...
    private boolean shouldRequestIdToken() {
        return ((CheckBox) binding.checkboxRequestIdtoken).isChecked();
    }
    ...
    private void showProgress() {
        ...
        binding.buttonLoadCredentials.setEnabled(false); 
        binding.buttonLoadHint.setEnabled(false);
        binding.buttonSaveCredential.setEnabled(false);
        binding.buttonDeleteLoadedCredential.setEnabled(false);
    }
    ...
    private void hideProgress() {
        ...
        binding.buttonLoadCredentials.setEnabled(true);
        binding.buttonLoadHint.setEnabled(true);
        binding.buttonSaveCredential.setEnabled(true);
    }

    @Override
    public void onClick(View v) {
        if(v.getId() == R.id.button_save_credential) {
               saveCredentialClicked();
        }else if(v.getId() == R.id.button_load_credentials) {
               loadCredentialsClicked();
        }else if(v.getId() == R.id.button_load_hint) {
               loadHintClicked();
        }else if(v.getId() == R.id.button_delete_loaded_credential) {
